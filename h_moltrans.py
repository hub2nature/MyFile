...
def smiles_to_graph(smiles, max_atoms=128):
    ...
    if x.shape[0] < max_atoms:
        pad_x = torch.zeros((max_atoms - x.shape[0], x.shape[1]))
        x = torch.cat((x, pad_x), dim=0)
        pad_adj = torch.zeros((max_atoms - adj.shape[0], adj.shape[1]))
        adj = torch.cat((adj, pad_adj), dim=0)
        pad_adj = torch.zeros((adj.shape[0], max_atoms - adj.shape[1]))
