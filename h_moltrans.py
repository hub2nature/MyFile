...
if os.path.exists("davis_no_weighted_5runs.txt"):
    os.remove("davis_no_weighted_5runs.txt")
sys.stdout = open('davis_no_weighted_5runs.txt', 'w')
...
def test_with_optimal_threshold(data_generator, model, pos_weight):
    ...
    with torch.no_grad():
        for d, p, d_mask, p_mask, label in data_generator:
            ...
            loss_fct = torch.nn.BCEWithLogitsLoss() #torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor([pos_weight], device= device))
...
def main(fold_n, lr):
    ...
    for epo in range(100):  # Epochs set to 100
        ...
        for i, (d, p, d_mask, p_mask, label) in enumerate(training_generator):
            ...
            loss_fct = torch.nn.BCEWithLogitsLoss() #torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor([pos_weight_train], device= device))
